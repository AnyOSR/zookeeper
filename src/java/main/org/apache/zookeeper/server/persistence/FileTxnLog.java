 ...
 * <blockquote><pre>
 * LogFile:                               log文件：文件头 事务日志 0填充
 *     FileHeader TxnList ZeroPad
 *
 * FileHeader: {                          文件头：魔数 版本号 dbid
 *     magic 4bytes (ZKLG)
 *     version 4bytes
 *     dbid 8bytes
 *   }
 ...
 *
 * ZeroPad:
 *     0 padded to EOF (filled during preallocation stage)    事务日志
 * </pre></blockquote>
 */
public class FileTxnLog implements TxnLog {
     ...
     * @throws IOException
     */
    // 当logStream为null时，下次append会创建一个新的文件
    ...
    public synchronized boolean append(TxnHeader hdr, Record txn) throws IOException {
        ...
        if (logStream==null) {                  // 如果logStream为null，会创建一个新文件，且新文件命名包含有第一个事务的zxid
           ...
           fhdr.serialize(oa, "fileheader");                                         // 写入文件头
           // Make sure that the magic number is written before padding.
           logStream.flush();
           filePadding.setCurrentSize(fos.getChannel().position());                       // 改变当前文件的写入位置
           streamsToFlush.add(fos);                                                       // 将当前打开的文件添加到streamsToFlush
        }
        filePadding.padFile(fos.getChannel());                                            // 填充文件，如果有必要的话
        ...
        oa.writeLong(crc.getValue(), "txnEntryCRC");                                  // 写入校验和
        Util.writeTxnBytes(oa, buf);                                                       // 写入事务日志以及EOR 有length？
     ...
     * @return
     */
    // 找出起始zxid大于等于snapshotZxid的log文件，有可能小于snapshotZxid
    public static File[] getLogFiles(File[] logDirList,long snapshotZxid) {
        List<File> files = Util.sortDataDir(logDirList, LOG_FILE_PREFIX, true);
        long logZxid = 0;  //记录出现的最大的小于等于snapshotZxid的zxid 这样就不会错过snapshotZxid
        // Find the log file that starts before or at the same time as the
        // zxid of the snapshot
        for (File f : files) {
            long fzxid = Util.getZxidFromName(f.getName(), LOG_FILE_PREFIX);   //找到当前文件的起始zxid
        ...
        for (FileOutputStream log : streamsToFlush) {
            ...
            if (forceSync) {
                ...
                if (syncElapsedMS > fsyncWarningThresholdMS) {
                    LOG.warn("fsync-ing the write ahead log in " + Thread.currentThread().getName() + " took " + syncElapsedMS
                            + "ms which will adversely effect operation latency. " + "File size is " + channel.size() + " bytes. "
    ...
    /**
     * truncate the current transaction logs      truncate文件至zxid
    ...
    public boolean truncate(long zxid) throws IOException {
        ...
        try {
            ...
            long pos = input.getPosition();           // 文件读取位置
     ...
     * for the application.
     */
    static class PositionInputStream extends FilterInputStream {    // 保存了实际读到的位置
        long position;
    ...
    public static class FileTxnIterator implements TxnLog.TxnIterator {
        ...
        public FileTxnIterator(File logDir, long zxid, boolean fastForward) throws IOException {
            ...
            // 如果是fastForward 则快速定位到第一个大于等于zxid的事务日志
         ...
         * new file to be read
         * @throws IOException         跳到下一个log文件并读取魔数
         */
        private boolean goToNextLog() throws IOException {
            if (storedFiles.size() > 0) {
                this.logFile = storedFiles.remove(storedFiles.size()-1);    //拿到最后一个log文件 所以其zxid最小
        ...
        protected InputArchive createInputArchive(File logFile) throws IOException {
            if(inputStream==null){
                ...
                inStreamCreated(ia,inputStream);                           // 检查文件头魔数
        ...
        public boolean next() throws IOException {
            ...
            try {
                long crcValue = ia.readLong("crcvalue");                // 读校验码
                byte[] bytes = Util.readTxnBytes(ia);                        // 读取一个事务日志
                ...
                if (crcValue != crc.getValue())                              // 校验校验码
                    throw new IOException(CRC_ERROR);
                hdr = new TxnHeader();                                       // 给hdr和record重新赋值
