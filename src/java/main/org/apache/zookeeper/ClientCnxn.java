...
/**
 * This class manages the socket i/o for the client. ClientCnxn maintains a list      这个类用来处理客户端IO，ClientCnxn维护了一个可用的server列表
 * of available servers to connect to and "transparently" switches servers it is      如果有必要的话，会无感知的切换server
 * connected to as needed.
 */
public class ClientCnxn {
    private static final Logger LOG = LoggerFactory.getLogger(ClientCnxn.class);

    /* ZOOKEEPER-706: If a session has a large number of watches set then              如果一个session拥有许多watch，在连接失效之后试着重建这些watch可能会失败
     * attempting to re-establish those watches after a connection loss may            因为 setWatch请求 会超过server配置的最大jute.maxBuffer值
     * fail due to the SetWatches request exceeding the server's configured
     * jute.maxBuffer value. To avoid this we instead split the watch                  为了避免这个，通过多次setwatch调用来完成这个重建过程
     * re-establishement across multiple SetWatches calls. This constant               下面的这个常量控制每次调用的大小控制
     * controls the size of each call. It is set to 128kB to be conservative            保守设置为128KB，server的jute.maxBuffer为1MB
    ...
    /**
     * Is set to true when a connection to a r/w server is established for the     第一次连接到读写server时设置为true
     * first time; never changed afterwards.
     * <p>
     * Is used to handle situations when client without sessionId connects to a     用于处理 没有session的client连接到只读server
     * read-only server. Such client receives "fake" sessionId from read-only       这些client收到来自server的假sessionId，
     * server, but this sessionId is invalid for other servers. So when such       这些sessionid对于其他server来说是无效的
     * client finds a r/w server, it sends 0 instead of fake sessionId during      因此，当这个client找到一个 r/w server时，它发送0而不是那个假的sessionId
     * connection handshake and establishes new, valid session.                    在它连接握手和建立一个新的有效的session的时候
     * <p>
     * If this field is false (which implies we haven't seen r/w server before)     如果这个字段值是false，意味着之前还没有遇到过 rw server
     * then non-zero sessionId is fake, otherwise it is valid.                      那么非0的sessionId就是一个假的，否则他是有效的
     */
    ...
    /**
     * Set to true when close is called. Latches the connection such that we              close被调用的时候设置为true，
     * don't attempt to re-connect to the server if in the middle of closing the          防止重新连接
    ...
    private static class WatcherSetEventPair {
        private final Set<Watcher> watchers;   // 该事件应该被触发的watch
    ...
    class EventThread extends ZooKeeperThread {
        ...
        private void queueEvent(WatchedEvent event, Set<Watcher> materializedWatchers) {
            ...
            sessionState = event.getState();    // 当前event的状态
        ...
        public void queuePacket(Packet packet) {
            if (wasKilled) {
                synchronized (waitingEvents) {
                    if (isRunning) {                 // 如果已经接收到毒丸，且waitingEvents还没有空过，则继续入队
                    ...
                    if (wasKilled)
                        synchronized (waitingEvents) {
                            if (waitingEvents.isEmpty()) {
                                isRunning = false;           // isRunning为true之后，不能继续queue packet
                                break;
                            }
                        }
                }
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
            ...
            replyHdr.deserialize(bbia, "header");            // 反序列化header
